...
public:
     ...
     */
    shader_program(std::string_view name, std::initializer_list<std::pair<shader_type, std::filesystem::path>> shaders) noexcept;
...
public:
    ...
    /**
     * @brief Upload a 1-4D unsigned int to the shader program
     *
     * @param name Uniform name.
     * @param vec
     */
    void upload_uniform(std::string_view name, const glm::uvec1& vec);
    void upload_uniform(std::string_view name, const glm::uvec2& vec);
    void upload_uniform(std::string_view name, const glm::uvec3& vec);
    void upload_uniform(std::string_view name, const glm::uvec4& vec);
...
inline shader_program::shader_program(std::string_view name,
    std::initializer_list<std::pair<shader_type, std::filesystem::path>> shaders) noexcept
    : m_id(-1)
    , m_name { name }
{
  for (const auto &[type, path] : shaders)
    m_shaders.push_back({type, util::read_file(path.string())});
...
inline void shader_program::upload_uniform(std::string_view name, const glm::uvec1& vec)
{
  glUniform1uiv(uniform_location(name), 1, glm::value_ptr(vec));
}

inline void shader_program::upload_uniform(std::string_view name, const glm::uvec2& vec)
{
  glUniform2uiv(uniform_location(name), 1, glm::value_ptr(vec));
}

inline void shader_program::upload_uniform(std::string_view name, const glm::uvec3& vec)
{
  glUniform3uiv(uniform_location(name), 1, glm::value_ptr(vec));
}

inline void shader_program::upload_uniform(std::string_view name, const glm::uvec4& vec)
{
  glUniform4uiv(uniform_location(name), 1, glm::value_ptr(vec));
}
